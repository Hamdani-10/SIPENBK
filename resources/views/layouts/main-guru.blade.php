...
<body class="bg-white min-h-screen font-[Poppins] antialiased" x-data="{ open: false, showLogoutModal: false }">
    {{-- Navbar --}}
    <nav id="navbar" x-data="{ open: false }" class="w-full z-50 fixed top-0 left-0 bg-transparent py-4 md:py-6 transition-all duration-300">
        <div class="max-w-7xl mx-auto flex items-center justify-between px-4 md:px-8">
            ...
            <div class="hidden md:flex items-center space-x-6 text-sm font-medium">
                @php
                    $navItems = [
                        ['label' => 'Beranda', 'route' => 'guru.dashboard.index'],
                        ['label' => 'Jadwal BK', 'route' => 'guru.jadwal.index'],
                        ['label' => 'Laporan BK', 'route' => 'guru.rekap.index'],
                        ['label' => 'Siswa', 'route' => 'guru.siswa.index'],
                        ['label' => 'Panduan', 'route' => 'guru.panduan.index'],
                    ];
                @endphp
            ...
            <div x-data="{ open: false }" class="relative hidden md:block">
                ...
                <button @click="open = !open" @keydown.escape="open = false" @click.outside="open = false"
                    class="flex items-center gap-3 bg-white px-4 py-2 border border-gray-200 md:px-5 md:py-3 rounded-full transition-all duration-300 w-full focus:outline-none">
                    
                    {{-- Foto Profil --}}
                    <div class="w-10 h-10 rounded-full overflow-hidden border border-gray-300">
                        <img src="{{ Auth::user()->guruBk && Auth::user()->guruBk->foto 
                            ? asset('storage/' . Auth::user()->guruBk->foto) 
                            : asset('assets/default-user.png') }}"
                            alt="Foto Profil" class="w-full h-full object-cover object-top" loading="lazy"
                    ...
                    <div class="hidden md:block text-left">
                        <p class="text-sm font-medium text-gray-900 leading-none">
                            {{ Auth::user()->guruBk->nama_user ?? 'Nama tidak tersedia' }}
                        </p>
                        <p class="text-xs text-gray-500">
                            {{ Auth::user()->guruBk->jabatan ?? Auth::user()->role }}
                        </p>
                    </div>
                ...
                <div x-show="open" x-transition:enter="transition ease-out duration-200"
                    ...
                    <div class="py-2">
                        ...
                        <form id="logout-form-mobile" method="POST">
            ...
            <button @click="open = !open" class="md:hidden focus:outline-none z-50 ml-2" aria-label="Toggle Navigation" aria-expanded="open" role="button">
        ...
        {{-- Overlay transparan untuk menutup sidebar saat klik di luar --}}
        <div class="fixed inset-0 z-30 cursor-default"
            x-show="open"
        ...
        {{-- Sidebar Mobile Menu --}}
        <div class="fixed top-0 left-0 h-full w-4/5 max-w-xs bg-white shadow-2xl transform transition-transform duration-300 ease-in-out z-40 flex flex-col justify-between rounded-r-2xl border-r border-gray-200" :class="{ 'translate-x-0': open, '-translate-x-full': !open }" x-cloak>
            ...
            <div class="p-6 space-y-6">
                ...
                <div class="flex items-center gap-3 bg-gray-100 rounded-xl px-4 py-3 shadow-inner">
                    ...
                    <div class="flex flex-col">
                        <span class="text-sm font-semibold text-gray-800">{{ Auth::user()->guruBk->nama_user ?? 'Nama tidak tersedia' }}</span>
                        <a href="{{route('guru.profil.index')}}" class="text-xs text-blue-600 hover:underline font-medium">Profil Saya</a>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-25">
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        <li><a href="{{route ('guru.dashboard.index')}}" class="hover:text-white transition">Beranda</a></li>
                        <li><a href="{{route ('guru.siswa.index')}}" class="hover:text-white transition">Siswa</a></li>
                        <li><a href="{{route ('guru.jadwal.index')}}" class="hover:text-white transition">Jadwal BK</a></li>
                        <li><a href="{{route ('guru.rekap.index')}}" class="hover:text-white transition">Laporan BK</a></li>
                        <li><a href="{{route ('guru.panduan.index')}}" class="hover:text-white transition">Panduan Sistem</a></li>
                    </ul>
                </div>
            </div>
    ...
    <div x-cloak>
        <div x-show="showLogoutModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/10 backdrop-blur-sm" x-transition x-cloak>
            <div class="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                ...
                <p class="text-gray-600 mb-5">Apakah kamu yakin ingin logout dari sistem?</p>
